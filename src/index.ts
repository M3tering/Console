...
import { scheduleVerificationJobs, runVerificationJobsManually } from "./logic/jobs";
...
app.post("/", async (req: Request, res: Response) => {
  try {
    ...
    saveMeter({ publicKey, tokenId, contractId });
  } catch (err) {
    console.error(err);
  }
  res.redirect("/");
...
app.post("/run-verification-jobs", async (req: Request, res: Response) => {
  try {
    ...
    console.log("[server]: Server handled POST request at `/run-verification-jobs`");
  } catch (err) {
    console.error(err);
