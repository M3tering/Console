...
export async function sendUnverifiedTransactionsJob(): Promise<void> {
  try {
    console.log(
      "🔄 Starting daily job: Send unverified transactions to verifier"
    );

    // Get unverified transactions from database
    const unverifiedTransactions = getUnverifiedTransactionRecords();
    ...
    const success = await sendTransactionsToVerifier(
    ...
    if (success) {
      ...
      console.error(
        "❌ Daily job failed: Could not send transactions to verifier"
      );
    }
  } catch (err: any) {
...
export async function checkVerificationStatusJob(): Promise<void> {
  try {
    ...
    const transactionIds = unverifiedTransactions.map((tx) => tx.nonce);
    const verificationResults = await checkVerificationStatus(
      transactionIds,
      selectedNode
    );
...
export function scheduleVerificationJobs(): void {
  ...
  console.log(
    "  - Daily at 02:00 UTC: Send unverified transactions to verifier"
  );
